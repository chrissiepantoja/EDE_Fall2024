...
knitr::kable(epa_data,align = c("r", "l"), caption = "EPA Air Quality")
```
...
```{r project.setup, echo = TRUE, error= FALSE, message=FALSE, warning=FALSE}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
summary_Ozone <- epa_data %>% 
  drop_na(Ozone) %>% 
  group_by(COUNTY) %>% 
  summarize(mean_Ozone = round(mean(Ozone), 2)) %>% 
  arrange(mean_Ozone)

# Display tables using knitr::kable
knitr::kable(summary_PM25, col.names = c("County", "$\\mu g/m^3$"), 
             align = c("l", "r"), caption = "Mean Particulates (2.5mm)")

knitr::kable(summary_Ozone, col.names = c("County", "$\\mu g/m^3$"), 
             align = c("l", "r"), caption = "Mean Ozone")
```
...
```{r ozone_boxplot_2018, echo=FALSE, warning = FALSE, fig.cap="2018 monthly Ozone levels", fig.height=3}
# Load necessary library
library(ggplot2)

# Create boxplot for Ozone levels in 2018 with a gray background and white grid lines
ggplot(epa_data %>% filter(year(Date) == 2018), aes(x = factor(month.abb[month(Date)], levels = month.abb), y = Ozone)) +
  geom_boxplot() +
  labs(title = "Monthly Ozone concentrations (2018)", x = "Month", y = "Concentration (µg/m³)") +
  theme_minimal() +
  theme(
    ...
    panel.grid.minor = element_line(color = "white"),   # Thinner white minor grid lines
    axis.ticks = element_line(color = "black")                      # Add ticks with gray color
  )
```

```{r ozone_boxplot_2019, echo=FALSE, warning = FALSE, fig.cap="2019 monthly Ozone levels", fig.height=3}

# Create boxplot for Ozone levels in 2019 with a gray background and white grid lines
ggplot(epa_data %>% filter(year(Date) == 2019), aes(x = factor(month.abb[month(Date)], levels = month.abb), y = Ozone)) +
  geom_boxplot() +
  labs(title = "Monthly Ozone concentrations (2019)", x = "Month", y = "Concentration (µg/m³)") +
  theme_minimal() +
  theme(
    plot.margin = margin(0.5, 0.5, 0.5, 0.5),
    panel.background = element_rect(fill = "gray90", color = NA),   # Gray panel background
...
https://github.com/chrissiepantoja/EDE_Fall2024
